...
def get_path_figure_from_data(data, count):
    ...
    plt.gcf().text(0.45, 0.91, str(count) + " of last " + str(NUMBER_OF_LAST_OCCURRENCES_TO_PLOT), fontsize=14)
    # draw gridlines
...
#        buttons_canvas = QPushButton(str(buttonsCounter), ui)
...
#        vbox.addWidget(buttons_canvas)

#    for path in succesfull_paths:
#        vbox.addWidget(path.canvas)
#        path.canvas.hide()
    vbox.addWidget(most_frequent_paths_figure.canvas)
    ui.setLayout(vbox)
    ui.show()
    sys.exit(app.exec_())
