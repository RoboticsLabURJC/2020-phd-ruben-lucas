...
class InferenceExecutorFactory:
    def __new__(cls, config):
        ...
        if agent == AgentsType.ROBOT_MESH.value:
            from rl_studio.agents.robot_mesh.inference_qlearn import QLearnRobotMeshInferencer
        ...
        #
            ...
            if algorithm == AlgorithmsType.DQN.value:
                from rl_studio.agents.cartpole.inference_dqn import DQNCartpoleInferencer as CartpoleInferencer
            else:
                from rl_studio.agents.cartpole.inference_qlearn import QLearnMountainCarInferencer as CartpoleInferencer

            return CartpoleInferencer(config)

        elif agent == AgentsType.MOUNTAIN_CAR.value:
            from rl_studio.agents.mountain_car.inference_qlearn import QLearnMountainCarInferencer
