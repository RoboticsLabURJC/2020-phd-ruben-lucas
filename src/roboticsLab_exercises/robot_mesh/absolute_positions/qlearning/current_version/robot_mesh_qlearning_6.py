...
mydoc = docx.Document()
p1=mydoc.add_paragraph()
p2=mydoc.add_paragraph()
p3=mydoc.add_paragraph()
p4=mydoc.add_paragraph()
p5=mydoc.add_paragraph()
r1 = p1.add_run()
r2 = p2.add_run()
r3 = p3.add_run()
r4 = p4.add_run()
r5 = p5.add_run()
rndmNum=np.random.rand()
...
class QSolver:
    ...
    def __init__(self):
        ...
        r1.add_text("GAMMA = " + str(GAMMA))
        r2.add_text("LEARNING_RATE = " + str(LEARNING_RATE))
        r3.add_text("EXPLORATION_DECAY = " + str(EXPLORATION_DECAY))
        r4.add_text("EXPLORATION_MIN = " + str(EXPLORATION_MIN))
        mydoc.save("./results_" + str(rndmNum) + ".docx")
...
def get_path_figure_from_data(data, count, figureId):
    ...
    if figureId != 1000:
        plt.savefig('figure_' + str(figureId) + '.png')
        r5.add_picture('./figure_' + str(figureId) + '.png')
        mydoc.save("./results_" + str(rndmNum) + ".docx")
        os.remove('./figure_' + str(figureId) + '.png')
...
def get_last_successfull_paths_figure(occurences):
    ...
    for element in occurences.keys():
        fig = get_path_figure_from_data(element, occurences.get(element),i)
        figures.append(fig)
        i+=1
    ...
    while True:
        ...
        while True:
            ...
            if q_solver.exploration_rate==EXPLORATION_MIN:
                ...
                most_frequent_paths_figure=get_path_figure_from_data(most_frequent_occurrence, count,1000)
