...
def _extend_contour(contour, target_y, direction, num_points_for_fit=70):
    """
    ...
    """
    if contour is None or len(contour) < 300:
        return contour
...
def _find_and_draw_lane_boundaries(binary_mask, save_path_intermediate_dir=None, base_name=None, reference_point=None, drivable_mask=None):
    ...
    # (has more than 6 neighbors in its 5x5 radius) AND is in the upper part of the image.
    # This threshold is loose enough to catch smooth merges but avoids splitting near the vehicle.
    
    # Create a 2D mask for the height condition to ensure correct broadcasting
    height_mask = np.zeros_like(skeleton, dtype=bool)
    height_mask[:int(height // 1.5), :] = True

    # Combine all three conditions (all are now 2D arrays)
    junction_mask = (neighbor_count > 6) & (skeleton > 0) & height_mask
    junction_points_neighbor = np.argwhere(junction_mask).tolist()
    ...
    for i in range(1, num_labels):
        ...
        if is_u_shape and peak_y < height // 1.5:
            junction_points_u_shape.append([peak_y, peak_x])
    
    junction_points = junction_points_neighbor + junction_points_u_shape
    ...
    for lane in lane_candidates:
        points = lane['points']
        points = points[points[:, 1].argsort()] # Sort by y-coordinate
        y_pts, x_pts = points[:, 1], points[:, 0]

        # Use a more accurate LOCALIZED fit on the bottom segment for classification
        num_points_for_classification_fit = 30
        if len(y_pts) > 2:
            try:
                fit_points_y = y_pts[-num_points_for_classification_fit:]
                fit_points_x = x_pts[-num_points_for_classification_fit:]
                # Use a robust linear fit for this bottom segment
                fit = np.polyfit(fit_points_y, fit_points_x, 1)
                local_fit_fn = np.poly1d(fit)
                x_bottom = local_fit_fn(y_bottom) # Extrapolate from the local fit
            except (np.linalg.LinAlgError, TypeError):
                 x_bottom = lane['fit_fn'](y_bottom) # Fallback to original global fit
        else:
            x_bottom = lane['fit_fn'](y_bottom) # Not enough points, use global fit

        # Draw the bottom point used for classification to help debug switching issues
        point_color = (255, 100, 0) if x_bottom < expected_center_x else (0, 100, 255)
        cv2.circle(candidates_viz, (int(x_bottom), y_bottom), 7, point_color, -1)
    ...
    # Any contour that reaches this point is guaranteed to be long enough due to the initial filter.
    if best_left_contour is not None:
        ...
        # Draw the extended left line for debugging on the candidates image
        cv2.polylines(candidates_viz, [best_left_contour.reshape((-1, 1, 2))], isClosed=False, color=(255, 255, 0), thickness=2, lineType=cv2.LINE_AA) # Cyan for extended left
    ...
    if best_right_contour is not None:
        ...
        # Draw the extended right line for debugging on the candidates image
        cv2.polylines(candidates_viz, [best_right_contour.reshape((-1, 1, 2))], isClosed=False, color=(255, 0, 255), thickness=2, lineType=cv2.LINE_AA) # Magenta for extended right
