...
class LoadAlgorithmParams:
    ...
    def __init__(self, config):
        ...
        if algorithm == "a2c":
            ...
            self.std_dev = config["algorithm"]["a2c"]["std_dev"]
            self.model_name = config["algorithm"]["a2c"]["model_name"]
            self.episodes_update = config["algorithm"]["a2c"]["episodes_update"]
            self.actor_lr = config["algorithm"]["a2c"]["actor_lr"]
            self.critic_lr = config["algorithm"]["a2c"]["critic_lr"]
            self.epsilon = config["algorithm"]["a2c"]["epsilon"]
...
class LoadGlobalParams:
    ...
    def __init__(self, config):
        ...
        self.net_arch = config["settings"].get("net_arch", [128, 128, 128, 128, 128])
...
class LoadEnvVariablesA2CCarla:
    """
    ONLY FOR a2c algorithm
    Creates a new variable 'environment', which contains values to Gazebo env, Carla env ...
    """
    ...
    def __init__(self, config) -> None:
        ...
        # Training/inference
        self.environment["mode"] = config["settings"]["mode"]
        self.environment["retrain_a2c_tf_model_name"] = f"{config['retraining']['a2c']['retrain_a2c_tf_model_name']}"
        self.environment["inference_a2c_tf_actor_model_name"] = config["inference"][
            "a2c"
        ]["inference_a2c_tf_actor_model_name"]
        self.environment["inference_a2c_tf_critic_model_name"] = config["inference"][
            "a2c"
        ]["inference_a2c_tf_critic_model_name"]
        ...
        # Algorithm
        self.environment["critic_lr"] = config["algorithm"]["a2c"]["critic_lr"]
        self.environment["actor_lr"] = config["algorithm"]["a2c"]["actor_lr"]
        self.environment["model_name"] = config["algorithm"]["a2c"]["model_name"]
