...
class PeriodicSaveCallback(BaseCallback):
    def __init__(self, env, params, save_path, save_freq=2000, verbose=1):
    ...
    def _on_step(self) -> bool:
        ...
        if self.step_count % self.save_freq == 0:
            ...
            if self.last_model_path and os.path.exists(self.last_model_path):
                if self.verbose > 0:
                    print(f"Deleting old periodic model: {self.last_model_path}")
                os.remove(self.last_model_path)

            model_save_path = os.path.join(self.save_path, f"model_{self.step_count}_steps_{time.strftime('%Y%m%d-%H%M%S')}.zip")
            self.model.save(model_save_path.replace(".zip", "")) # .save adds the .zip
            
            # Store the new path to be deleted in the next cycle
            self.last_model_path = model_save_path
...
class ExplorationRateCallback(BaseCallback):
    ...
    def _on_training_start(self):
        ...
        if self.verbose > 0:
            logger.info(f"Training started: Initial exploration rates set to v={self.v_exploration_rate}, w={self.w_exploration_rate}")
    ...
    def _on_step(self) -> bool:
        ...
        if self.current_step % self.decay_steps == 0:
            ...
            self.tensorboard.update_stats(std_dev_w=self.w_exploration_rate)
            if self.verbose > 0:
                logger.info(f"Step {self.current_step}: Updated exploration rates to v={self.v_exploration_rate}, w={self.w_exploration_rate}")
...
class TrainerFollowLaneSACCarla:
    ...
    def main(self):
        ...
        all_config = self.tensorboard.combine_attributes(self.algoritmhs_params,
                                                         self.environment,
                                                         self.global_params)
        reward_filename = f"{os.getcwd()}/envs/carla/acc/acc_carla_sb_3.py"
        ...
        if self.global_params.steps_to_decrease < 100000:
            logger.info("Added exploration callback!")
